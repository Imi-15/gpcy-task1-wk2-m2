...
MODEL_BUNDLE_PATH = "fraud_model_bundle.joblib"


def save_model_bundle(model, feature_names, medians):
    bundle = {
        "model": model,
...
def load_model_bundle():
    if os.path.exists(MODEL_BUNDLE_PATH):
        ...
        return bundle.get("model"), bundle.get("feature_names"), bundle.get("medians")
...
def main():
    ...
    # Try to load CSV - prefer sample for deployment, fallback to full data or upload
    sample_path = "application_data_sample.csv"
    full_path = "application_data.csv"
    data_path = sample_path if os.path.exists(sample_path) else full_path
    df = None
    ...
    if os.path.exists(data_path):
        ...
        # Fallback: allow file upload if neither CSV found
        st.info("Please upload your data CSV file to get started.")
    ...
    model, feature_names, medians = load_model_bundle()
    if model is None or feature_names is None or medians is None:
        if df is None:
            st.error("No training data available to build the model.")
            return
        model, feature_names, medians = run_pipeline(df)
        save_model_bundle(model, feature_names, medians)
